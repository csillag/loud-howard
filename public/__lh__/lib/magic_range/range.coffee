...
window.magic.Range.sniff = (r) ->
  if r.commonAncestorContainer?
    new magic.Range.BrowserRange(r)
  else if typeof r.start is "string"
    new magic.Range.SerializedRange(r)
  else if r.start and typeof r.start is "object"
    new magic.Range.NormalizedRange(r)
  else
...
window.magic.Range.nodeFromXPath = (xpath, root=document) ->
...
class window.magic.Range.BrowserRange
  ...
  normalize: (root) ->
    ...
    new magic.Range.NormalizedRange(nr)
...
class window.magic.Range.NormalizedRange
...
#      console.log textNodes
    ...
    new magic.Range.SerializedRange({
      # XPath strings
      start: start[0]
      end: end[0]
...
class window.magic.Range.SerializedRange
  ...
  normalize: (root) ->
    ...
    for p in ['start', 'end']
      try
        node = magic.Range.nodeFromXPath(this[p], root)
      catch e
        throw new magic.Range.RangeError(p, "Error while finding #{p} node: #{this[p]}: " + e, e)

      if not node
        throw new magic.Range.RangeError(p, "Couldn't find #{p} node: #{this[p]}")
    ...
    new magic.Range.BrowserRange(range).normalize(root)
